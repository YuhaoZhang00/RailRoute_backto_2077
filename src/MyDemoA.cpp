...
void MyDemoA::virtSetupBackgroundBuffer()
{
	...
	image.renderImage(getBackgroundSurface(), 0, 0, 30, 100, image.getWidth(), image.getHeight());
	image.renderImage(getBackgroundSurface(), 30, 40, 300, 100, 50, 50);
...
void MyDemoA::virtMouseDown(int iButton, int iX, int iY)
{
	...
	if (iButton == SDL_BUTTON_LEFT)
	{
		...
		lockBackgroundForDrawing();
		drawBackgroundOval(iX - 12, iY - 12, iX + 12, iY + 12, 0x00ff00);
		unlockBackgroundForDrawing();
		redrawDisplay();
	}
}
